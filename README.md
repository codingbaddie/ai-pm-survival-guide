...
> 指南中的所有案例與檢查點，皆來自於真實的 Agent 開發過程，經過反覆測試與驗證。我不談空泛理論，只分享那些讓 AI「已讀不回」的真實踩坑經驗。
...
| :--- | :--- | :--- |
| **連結處理** | ❌ **Smart Chips (智慧晶片)**<br>貼上網址後變成灰色的膠囊狀，API 讀取時往往會**遺失網址**，只剩標題。 | ✅ **標準超連結 (Hyperlink)**<br>使用藍色底線文字 (`Cmd+K`)，確保 `url` 屬性被保留。<br><br>👉 [範例連結](https://docs.google.com/) (O)<br>👉 `[📄 範例文件]` (X) |
| **分頁標示** | 🚫 **隱形的分頁 (Tabs)**<br>API 讀取會將所有分頁接成一串長文，AI 分不清邊界。 | ✅ **H1 標題路標**<br>在每個分頁內容的最頂端，手動加上 **H1 大標題** 寫下分頁名稱。<br>*(例：# Tab 1: 訪談紀錄)* |
...
*   **AI 聽不到聲音**：無法直接讀取 `.mp4` / `.mov` 影片檔。
*   **解法**：請提供 **逐字稿 (Transcript)** (以 `.txt` 格式或貼在 Doc 中)。
